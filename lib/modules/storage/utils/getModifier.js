...
const getModifier = async function(options = {}) {
  let {
    doc: newDoc,
    transient = false,
    immutable = false,
  ...
  if (!oldDoc) {
    // Use async method for Meteor v3 compatibility on the server
    oldDoc = Meteor.isServer 
      ? await Class.findOneAsync(newDoc._id, opts)
