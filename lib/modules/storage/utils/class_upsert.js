...
async function classUpsert(args = {}) {
  const {
    className,
    selector,
    modifier,
  ...
  // Use async count for Meteor v3 compatibility on the server
  const docsCount = Meteor.isServer ? await docs.countAsync() : docs.count();
  if (docsCount > 0) {
    await docs.forEachAsync(async (doc) => {
      ...
      // Update a document.
      result.numberAffected += await documentUpdate({
        doc,
        stopOnFirstError,
        simulation,
        fields,
    ...
    // Insert a document.
    result.insertedId = await documentInsert({
      doc,
      stopOnFirstError,
      simulation,
      trusted,
