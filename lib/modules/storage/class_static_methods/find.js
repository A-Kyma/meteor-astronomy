...
function createMethod(methodName) {
  return function(selector, options) {
    ...
    if (_isNull(selector)) {
      // On the server with Meteor 3, use async methods
      if (Meteor.isServer && methodName === 'findOne') {
        throw new Meteor.Error('sync-method-not-available', 
    ...
    // On the server with Meteor 3, findOne is not available
    if (Meteor.isServer && methodName === 'findOne') {
      throw new Meteor.Error('sync-method-not-available', 
        'findOne is not available on the server. Please use findOneAsync() instead.');
    }
...
function createMethodAsync(methodName) {
...
const findOneAsync = createMethodAsync('findOneAsync');


export { find, findOne, findOneAsync };